...
import { GoogleBook, ImportPayload, UserBook, UserProfile, ReadingSession } from './types';
...
import { BookClubView } from './views/BookClubView';
import { NexumView } from './views/NexumView';
...
import { 
  ...
  Trophy, Settings, Bell, ChevronDown, Share2, Network, Globe,
  PenTool, CircleUser, Waves, Timer, Database
} from 'lucide-react';

type View = 'importer' | 'quotes' | 'feed' | 'library' | 'my_shelf' | 'book_detail' | 'book_review' | 'vocabulary' | 'study_mode' | 'correspondence' | 'challenges' | 'nexum' | 'clubs' | 'aethel' | 'codex' | 'author_detail' | 'friends' | 'admin';
...
const App: React.FC = () => {
  ...
  const [activeSessionBook, setActiveSessionBook] = useState<UserBook | null>(null);
  const [currentAuthorName, setCurrentAuthorName] = useState<string | null>(null);
  ...
  const handleFinishSession = (session: ReadingSession) => {
    addToast(`Arhivirano ${session.durationMinutes}m dubokog čitanja.`);
  ...
  const navigateToAuthor = (name: string) => {
    ...
    setView('author_detail');
  };

  const handleOpenFriend = (friend: UserProfile) => {
    addToast(`Gledate biblioteku korisnika ${friend.name}`);
    setView('library'); 
  };

  const handleMessageFriend = (friend: UserProfile) => {
    addToast(`Započinjete korespodenciju sa ${friend.name}`);
  ...
  return (
    <div className="flex min-h-screen transition-colors duration-500 overflow-hidden bg-[var(--color-bg)]">
        ...
        <div className="p-8 overflow-y-auto custom-scrollbar">
          ...
          <nav className="space-y-1">
            ...
            <NavGroup label="Commonplace">
              <SidebarItem active={view === 'nexum'} onClick={() => setView('nexum')} label="The Nexum" icon={<Network size={18} />} />
              <SidebarItem active={view === 'aethel'} onClick={() => setView('aethel')} label="Aethel AI" icon={<Sparkles size={18} />} badge="Pro" />
            ...
            <NavGroup label="Creation">
              <SidebarItem active={view === 'codex'} onClick={() => setView('codex')} label="The Codex" icon={<PenTool size={18} />} badge="Beta" />
            </NavGroup>

            <NavGroup label="Exchange">
              <SidebarItem active={view === 'friends'} onClick={() => setView('friends')} label="The Resonance" icon={<Waves size={18} />} />
              <SidebarItem active={view === 'clubs'} onClick={() => setView('clubs')} label="Circles" icon={<Globe size={18} />} badge="New" />
            ...
            <NavGroup label="Systems">
              ...
              <SidebarItem active={view === 'admin'} onClick={() => setView('admin')} label="Architect Console" icon={<Database size={18} />} />
            </NavGroup>
          </nav>
        </div>
        ...
        <div className="mt-auto p-6 border-t border-[var(--color-border)]">
          <button className="w-full flex items-center gap-4 p-3 rounded-2xl hover:bg-[var(--color-border)] transition-all group">
            ...
            <div className="text-left">
              ...
              <p className="text-[10px] text-[var(--color-text-secondary)] font-bold uppercase tracking-widest">Architect Access</p>
            </div>
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden">
        ...
        <header className="h-20 border-b border-[var(--color-border)] flex items-center justify-between px-8 bg-[var(--color-surface)] backdrop-blur-xl z-40">
          ...
          <div className="flex items-center gap-6 ml-8">
            ...
            <button className="relative p-2.5 text-[var(--color-text-secondary)] hover:text-indigo-500 transition-colors">
              <Bell size={20} />
              <div className="absolute top-2 right-2 w-2 h-2 rounded-full bg-indigo-500 border-2 border-[var(--bg)]" />
            </button>
        ...
        <main className="flex-1 overflow-y-auto custom-scrollbar bg-[var(--color-bg)]">
          {view === 'admin' ? (
            ...
            <BookClubView onOpenBook={(id) => console.log(id)} />
          ) : view === 'friends' ? (
            <FriendsView onOpenFriend={handleOpenFriend} onMessage={handleMessageFriend} />
          ) : view === 'nexum' ? (
            <NexumView onOpenBook={(b) => { setCurrentBookDetail(b); setView('book_detail'); }} />
          ) : view === 'my_shelf' ? (
            ...
            <LibraryView 
              onOpenBook={(b) => { setCurrentBookDetail(b); setView('book_detail'); }} 
              onNavigateToImporter={() => setView('importer')} 
              onStartSession={(b) => setActiveSessionBook(b)} 
            />
          ) : view === 'quotes' ? (
            ...
            <CorrespondenceView />
          ) : view === 'author_detail' && currentAuthorName ? (
            <AuthorDetailView 
              authorName={currentAuthorName} 
              onBack={() => setView('library')} 
              onOpenBook={(b) => { setCurrentBookDetail(b); setView('book_detail'); }} 
              onOpenAuthor={(name) => navigateToAuthor(name)}
            />
            ...
            <BookDetailView 
              ...
              onAddWord={() => setIsVocabModalOpen(true)} 
              onEnterStudyMode={() => setView('study_mode')}
              onOpenAuthor={(name) => navigateToAuthor(name)}
              onStartSession={(b) => setActiveSessionBook(b as UserBook)}
            />
          ) : (
      ...
      {activeSessionBook && (
        <SessionView 
          book={activeSessionBook} 
          onClose={() => setActiveSessionBook(null)} 
          onFinish={handleFinishSession} 
        />
      )}

      {selectedBookPreview && <BookPreviewModal book={selectedBookPreview} onClose={() => setSelectedBookPreview(null)} onImport={(p) => { addToast(`Uvezeno ${p.book.volumeInfo.title}`); setView('my_shelf'); setSelectedBookPreview(null); }} onOpenDetail={() => { setCurrentBookDetail(selectedBookPreview); setView('book_detail'); setSelectedBookPreview(null); }} />}
