...
import { Star, Edit3, Trash2, Share2, Copy, Bookmark, MoreHorizontal, ExternalLink } from 'lucide-react';
...
export const QuoteCard: React.FC<QuoteCardProps> = ({ quote, onEdit, onDelete, onToggleFavorite, onTagClick, onOpenBook }) => {
  ...
  return (
    <div className="group relative glass bg-slate-900/40 rounded-3xl p-8 border border-slate-800/50 hover:border-indigo-500/50 transition-all duration-500 hover:shadow-2xl hover:shadow-indigo-500/5">
      ...
      <div className="relative space-y-6">
        ...
        {/* Book Info */}
        <div 
          className="flex items-start gap-4 pt-4 border-t border-slate-800/50 cursor-pointer hover:bg-slate-800/30 rounded-xl transition-colors group/book"
          onClick={() => onOpenBook?.(quote.bookTitle)}
        >
          {quote.bookCover ? (
          ...
          <div className="flex-1 min-w-0">
            <h4 className="text-white font-bold truncate text-lg group-hover/book:text-indigo-400 transition-colors flex items-center gap-2">
              {quote.bookTitle}
              <ExternalLink size={14} className="opacity-0 group-hover/book:opacity-100" />
            </h4>
          ...
          <div className="flex flex-wrap gap-2">
            {quote.tags.map((tag, i) => (
              <button 
                ...
                className="px-3 py-1 rounded-full bg-slate-800/50 border border-slate-700 text-xs font-semibold text-slate-400 hover:text-indigo-400 hover:border-indigo-500/30 transition-all dark:bg-slate-800/50 dark:text-slate-400"
              >
                #{tag}
              </button>
            ))}
