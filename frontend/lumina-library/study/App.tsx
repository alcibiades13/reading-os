...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 selection:bg-indigo-500/30">
      ...
      <nav className={`relative z-[60] border-b border-slate-900 bg-slate-950/50 backdrop-blur-md sticky top-0 ${view === 'book_review' || view === 'study_mode' ? 'hidden' : ''}`}>
        ...
        {toasts.map(toast => (
          <div 
            ...
            className={`pointer-events-auto border rounded-xl px-5 py-4 shadow-2xl flex items-center gap-3 animate-in slide-in-from-right-full duration-300 ${
              toast.type === 'emerald' ? 'bg-slate-900 border-emerald-500/30' : 
              toast.type === 'info' ? 'bg-slate-900 border-indigo-500/30' :
          ...
          >
            <div className={`w-6 h-6 rounded-full flex items-center justify-center ${
              ...
              <Sparkles size={14} className={toast.type === 'emerald' ? 'text-emerald-500' : 'text-indigo-400'} />
            </div>
