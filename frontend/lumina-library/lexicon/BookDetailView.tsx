...
export const BookDetailView: React.FC<BookDetailViewProps> = ({ book, onBack, onToast, onWriteReview, onAddWord }) => {
  // User Personal State
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
        ...
        <div className="lg:col-span-4 space-y-8">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            {/* Fixed: Used QuoteIcon instead of Quote type */}
            <MetaBox icon={<QuoteIcon size={18} className="text-indigo-400" />} label="Insights" value={`${bookQuotes.length} quotes`} />
          </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-8 border-t border-slate-800/50">
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="flex gap-4">
                       <button className="flex-1 py-4 rounded-xl border border-slate-700 text-slate-300 font-bold hover:bg-slate-800 transition-all flex items-center justify-center gap-3">
                         <Share2 size={18} />
                         Podeli
                       </button>
                       <button 
                         onClick={onAddWord}
                         className="flex-1 py-4 rounded-xl border border-emerald-500/30 bg-emerald-500/5 text-emerald-400 font-bold hover:bg-emerald-500/10 transition-all flex items-center justify-center gap-3"
                       >
                         <Brain size={18} />
                         Add Word
                       </button>
          ...
          <section className="space-y-6">
            ...
            {bookQuotes.length > 0 ? (
              ...
              <div className="p-12 rounded-3xl border-2 border-dashed border-slate-800 flex flex-col items-center text-center">
                ...
                <p className="text-slate-500 text-sm italic">Još uvek nisi sačuvao nijedan citat.</p>
              </div>
            )}
          </section>
        </div>
