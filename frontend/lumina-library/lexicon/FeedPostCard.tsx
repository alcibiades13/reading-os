...
export const FeedPostCard: React.FC<FeedPostCardProps> = ({ post, onOpenBook }) => {
  ...
  // Fixed: Added missing 'vocabulary' entry to satisfy Record<FeedActivityType, ...>
  const activityConfig: Record<FeedActivityType, { icon: any, label: string, color: string }> = {
    ...
    vocabulary: { icon: <Brain size={14} />, label: 'Expanded Lexicon', color: 'text-emerald-400 bg-emerald-400/10' }
  };
