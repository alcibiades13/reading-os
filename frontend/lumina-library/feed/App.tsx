...
type View = 'importer' | 'quotes' | 'feed';
...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 selection:bg-indigo-500/30">
      ...
      <nav className="relative z-[60] border-b border-slate-900 bg-slate-950/50 backdrop-blur-md sticky top-0">
              ...
              {!loading && results.length === 0 && !error && (
                <div className="flex flex-col items-center justify-center py-32 text-center opacity-50">
                  ...
                  <div className="mt-12 flex flex-wrap justify-center gap-3">
                    <span className="text-xs font-bold uppercase tracking-widest text-slate-600 w-full mb-2">Popular suggestions</span>
                    <SuggestionChip label="Atomic Habits" onClick={() => handleSearch('Atomic Habits', 'general')} />
                    <SuggestionChip label="Project Hail Mary" onClick={() => handleSearch('Project Hail Mary', 'general')} />
                    <SuggestionChip label="Dune" onClick={() => handleSearch('Dune', 'general')} />
                  </div>
                </div>
              )}
