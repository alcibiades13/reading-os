...
const stats = computed(() => {
  const totalPages = booksStore.books.reduce((acc, b) => acc + (b.book?.pageCount || 0), 0)
...
const getSpineColor = (bookId) => {
  ...
  // Use book ID to consistently assign same color
...
<template>
  <div class="max-w-7xl mx-auto px-6 py-10 animate-in fade-in duration-1000">
    ...
    <div class="space-y-40 pb-20">
      ...
      >
        ...
        <div class="flex items-center justify-between mb-8 px-4">
          ...
          <div class="flex items-center gap-3">
            <span class="px-4 py-1.5 rounded-full bg-white/5 border border-white/10 text-[9px] font-black text-slate-400 uppercase tracking-widest">
              {{ getShelfBooks(shelf).length }} VOLUMES
            </span>
        ...
        <div class="relative min-h-[300px] perspective-shelf">
          ...
          <div class="flex items-end gap-[4px] px-12 absolute bottom-[50px] left-0 right-0 z-20 overflow-visible">
            <div
              v-for="book in getShelfBooks(shelf)"
              :key="book.id"
              @click="handleOpenBook(book)"
              class="relative group cursor-pointer book-hover-effect flex flex-col items-center"
              :style="{ width: `${Math.max(20, Math.min(95, ((book.book?.pageCount || 250) / 250) * 45))}px` }"
            >
              <!-- Premium Tooltip -->
              <div class="absolute -top-20 opacity-0 group-hover:opacity-100 transition-all duration-300 pointer-events-none z-50 whitespace-nowrap bg-white text-slate-950 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-[0_20px_50px_rgba(0,0,0,0.5)] border border-white/20">
                {{ book.book?.title }}
              ...
              <div
                ...
                :style="{ height: `${Math.max(190, Math.min(240, 210 + ((book.book?.pageCount || 250) / 60)))}px` }"
              >
                ...
                <div class="absolute top-6 left-0 right-0 h-[1px] bg-black/30 pointer-events-none" />
                <div class="absolute bottom-6 left-0 right-0 h-[1px] bg-black/30 pointer-events-none" />
              ...
              <div class="absolute bottom-0 w-full h-2 bg-black/60 blur-[4px] opacity-40 group-hover:opacity-0 transition-opacity duration-500 pointer-events-none" />
            </div>
          </div>
          ...
          <div
            v-if="getShelfBooks(shelf).length === 0"
